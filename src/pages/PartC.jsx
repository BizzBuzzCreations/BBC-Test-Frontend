...
const PartC = () => {
  ...
  const sendMarksToBackend = async (marks) => {
    try {
      const res = await axios.post(
        "https://bbc-backend.bizzbuzzcreations.com/api/auth/add-marks",
        {
          testId: "Part-C",
          marks,
        },
  ...
  useEffect(() => {
    ...
    if (timeLeft === 0) {
      ...
      const acceptableAnswers = correctAnswers[currentQuestion].map((a) =>
        normalize(a),
      );
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#cfcbd1] px-4">
      <div className="bg-[#e9e9eb] w-full max-w-4xl rounded-2xl shadow-lg p-10 text-center">
        ...
        <p className="text-lg mb-6">
          Listen to 2 people have a conversation. Then answer 4 questions about
          the conversation.
        </p>
